...
#include <private/qcore_unix_p.h> // overrides QTQWSIntelCEIRKbPrivate_OPEN
...
class QWSIntelCEIRKbPrivate : public QObject, public IRReceiverCallback
{
    Q_OBJECT
    Q_PROPERTY(Qt::Key key_enum);

public:
    QWSIntelCEIRKbPrivate(QWSIntelCEKeyboardHandler *, const QString &);
    ~QWSIntelCEIRKbPrivate();
    ...
    QWSIntelCEKeyboardHandler    *m_handler;
...
QWSIntelCEKeyboardHandler::QWSIntelCEKeyboardHandler(const QString &device)
    : QWSKeyboardHandler(device)
{
    d = new QWSIntelCEIRKbPrivate(this, device);
}

QWSIntelCEKeyboardHandler::~QWSIntelCEKeyboardHandler()
{
    delete d;
}

int QWSIntelCEKeyboardHandler::KeyToKeyCode(QString keyName)
{
    return QWSIntelCEIRKbPrivate::KeyToKeyCode(keyName);
}

bool QWSIntelCEKeyboardHandler::filterInputEvent(quint16 &, qint32 &)
{
    return false;
}

QWSIntelCEIRKbPrivate::QWSIntelCEIRKbPrivate(QWSIntelCEKeyboardHandler *h, const QString &device)
    : m_handler(h)
{
    Q_UNUSED(device);
    ...
    setObjectName(QLatin1String("InteCE IR Keyboard Handler"));
...
QWSIntelCEIRKbPrivate::~QWSIntelCEIRKbPrivate()
{
    if (pIrReceiver != NULL)
    {
        pIrReceiver->Deinit();
...
int QWSIntelCEIRKbPrivate::KeyToKeyCode(QString keyName)
{
    const QMetaObject &mo = QWSIntelCEIRKbPrivate::staticMetaObject;
...
void QWSIntelCEIRKbPrivate::readKeycode()
{
...
void QWSIntelCEIRKbPrivate::processIREvent(const int nKeyCode, const int unicode, const unsigned char modifier, bool checkForDrop, void *pContext)
{
    ...
    else if (nKeyCode > 0)
    {
        ...
        {
            QWSIntelCEIRKbPrivate* kbPrivate = (QWSIntelCEIRKbPrivate*) pContext;
